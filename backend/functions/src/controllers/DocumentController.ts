...
STIL: Professionell, präzise, praxisorientiert für beide Jurisdiktionen`;
  ...
  /**
   * Analyze document against Swiss obligation law using direct PDF processing (Background Processing)
   * POST /api/documents/:documentId/analyze-swiss-obligation-law-direct
   */
  public async analyzeSwissObligationLawDirect(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = this.getUserId(req);
      const { documentId } = req.params;
      ...
      this.logger.info('Direct Swiss obligation law analysis requested', {
        userId,
        documentId
      });
      ...
      if (document.contentType !== 'application/pdf') {
        this.sendError(res, 400, 'Invalid file type', 'Direct analysis only supports PDF files');
        return;
      }

      // Create background job for direct analysis
      const jobId = await this.jobQueueService.createJob(
        'swiss-obligation-analysis-direct',
        userId,
        { documentId, userId, fileName: document.fileName }
      );

      this.logger.info('Direct Swiss obligation law analysis job created', {
        userId,
        documentId,
        jobId
      });
      ...
      this.sendSuccess(res, {
        ...
        documentId,
        status: 'processing',
        message: 'Direct PDF analysis started in background. You will receive a notification when completed.'
      }, 'Direct Swiss obligation law analysis started successfully');

    } catch (error) {
      this.logger.error('Direct Swiss obligation law analysis job creation failed', error as Error, {
        userId: this.getUserId(req),
        documentId: req.params.documentId
      });
      ...
      if (error instanceof Error) {
        if (error.message.includes('not found')) {
          ...
          this.sendError(res, 400, 'Invalid file type', error.message);
        } else {
          this.sendError(res, 500, 'Direct Swiss obligation law analysis failed', error.message);
        }
      } else {
        this.sendError(res, 500, 'Unexpected error during direct Swiss obligation law analysis');
      }
